...
- Bám sát dữ kiện từ bài nguồn. Tuyệt đối KHÔNG tự bịa đặt, không suy diễn sai lệch, không thêm số liệu hay sự kiện không có thực. Đảm bảo tính chính xác 100% thông tin.
...
"""
    ...
    def create_script_user_prompt(
        ...
        target_words = self._target_word_count(min_words, max_words)
        content = ContentCleanerTool.truncate_content(article.content, 6000)
...
Nội dung bài báo:
{ContentCleanerTool.truncate_content(article.content, 5000)}
...
Nội dung bài báo:
{ContentCleanerTool.truncate_content(article.content, 6000)}""",
        }
